...
export default function LocationsList({locations}) {
    ...
    const handleShowIndex = (id) => {
        router.get(`/${i18n.language}/${t('address.adminpanel')}/${t('address.locations')}/${id}`);
    };
    ...
    return (
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div
                className="hidden md:grid grid-cols-12 gap-4 px-8 py-4 bg-slate-50/50 border-bottom border-slate-100 text-slate-400 text-[11px] font-bold uppercase tracking-widest">
            ...
            <div className="divide-y divide-slate-50">
                {locations.map((location) => (
                    <div
                        key={location.id}
                        onClick={() => handleShowIndex(location.id)}
                        className="group grid grid-cols-1 md:grid-cols-12 gap-4 px-6 md:px-8 py-5 items-center hover:bg-slate-50/80 transition-all duration-200 cursor-pointer"
                    >
                        <div className="col-span-4 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <span
                                    className="text-[10px] text-slate-400 font-medium px-2 py-0.5 bg-slate-100 rounded-full mt-1 inline-block">
