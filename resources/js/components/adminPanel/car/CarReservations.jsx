                ...
                {currentReservations.map((r, index) => (
                    <tr key={startIndex + index} onClick={() => setSelectedReservation(r)}
                        ...
                        <Td
                            ...
                            contents={[
                                r.name + " " + r.surname,
                                new Date(r.pickup_datetime).toLocaleString("tr-TR", {
                                    day: "numeric",
                                    month: "numeric",
                                    year: "numeric",
                                    hour: "2-digit",
                                ...
                                new Date(r.return_datetime).toLocaleString("tr-TR", {
                                    day: "numeric",
                                    month: "numeric",
                                    year: "numeric",
                                    hour: "2-digit",
                                ...
                                <span
                                    className={`${r.payment_status.toLowerCase() === 'paid' ? 'text-green-600' : r.payment_status.toLowerCase() === 'failed' ? 'text-red-600' : 'text-yellow-600'} font-semibold`}
                                    key="odeme">
                    {r.payment_status}
                  </span>,
                                <span
                                    className={`font-semibold ${r.status.toLocaleLowerCase() === "pending" ? "text-yellow-600" : r.status.toLocaleLowerCase() === "confirmed" ? "text-green-600" : r.status.toLocaleLowerCase() === "cancelled" ? "text-red-600" : "text-black"} `}>
                      {r.status}
                  </span>,
                            ]}
                        />
