...
export default function CarPriceDetailForm({ data, setData, errors, setErrors }) {
    ...
    const validateDays = useCallback((value, dayIndex) => {
        ...
        for (let i = 0; i < dayKeys.length; i++) {
            ...
            if (!other) continue;
            if (current.max === Infinity && other.max === Infinity)
                return { error: "İki sonsuz değer olamaz" };
            if (current.min <= other.max && current.max >= other.min)
                return { error: `${dayKeys[i]} ile çakışıyor` };
        }
        return { error: "" };
    }, [dayKeys, parseRange]);
    ...
    const handleDayKeyChange = useCallback((oldKey, newKey, index) => {
        ...
        setErrors(prev => ({
            ...prev,
            month: { ...prev.month, [index]: validation.error }
        }));
    ...
    const updateDayKey = useCallback((oldKey, newKey) => {
        setData(prev => {
            ...
            Object.entries(prev.price).forEach(([month, daysObj]) => {
                const updatedDays = {};
                Object.entries(daysObj).forEach(([dayKey, value]) => {
                    updatedDays[dayKey === oldKey ? newKey : dayKey] = value;
                });
    ...
    const addColumn = useCallback(() => {
        ...
        setData(prev => {
            ...
            for (let month = 1; month <= 12; month++) {
                newPrice[month] = { ...newPrice[month], [newKey]: "" };
            }
            return { ...prev, price: newPrice };
        });
    }, [setData]);
    ...
    const removeColumn = useCallback((dayKey) => {
        if (dayKeys.length <= 1) {
            alert("En az bir sütun bulunmalıdır");
            return;
        }
        ...
        setData(prev => {
            const newPrice = {};
            for (let month = 1; month <= 12; month++) {
                const { [dayKey]: _, ...rest } = prev.price[month];
                newPrice[month] = rest;
            }
            return { ...prev, price: newPrice };
        });
    }, [dayKeys.length, setData]);

    const validatePrice = (e, i, dayKey) => {
        let err = "";
        if (e.target.value.trim() && !/^\d+(\.\d{1,2})?$/.test(e.target.value))
            err = "Geçersiz fiyat";
        setErrors(prev => ({
            ...prev,
            price: {
            ...
            <div className="h-full flex items-center justify-center p-4 overflow-x-auto">
                <table className="table-fixed border-separate border-spacing-4 rounded-md">
                    ...
                    <tr className="h-auto">
                        ...
                        <th><button type="button" onClick={addColumn} className="px-4 py-2 bg-green-500 rounded-md text-white hover:bg-green-600 cursor-pointer transition-colors" aria-label="Add new column">+</button></th>
                    </tr>
                    </thead>
                    <tbody>
                    {months.map((month, i) => (
