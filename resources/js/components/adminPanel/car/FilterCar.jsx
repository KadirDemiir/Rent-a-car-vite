...
export default function FilterCar({cars, setFilteredCars}) {
    ...
    const [brandOptions, setBrandOptions] = useState(() => cars.map(car => car.brand));
    ...
    const filterInput = (search) => {
        ...
        const brandMatches = cars.filter(car => car.brand.toLowerCase().includes(lowerSearch) && !plakaMatches.includes(car));
        const modelMatches = cars.filter(car => car.model.toLowerCase().includes(lowerSearch) && !plakaMatches.includes(car) && !brandMatches.includes(car));
        const segmentMatches = cars.filter(car => car.segment.toLowerCase().includes(lowerSearch) && !plakaMatches.includes(car) && !brandMatches.includes(car) && !modelMatches.includes(car));
            ...
            <div ref={wrapperRef} className="relative inline-block">
                ...
                {openFilter && (
                    <div className="absolute top-full left-0 mt-2 bg-white border border-gray-300 rounded p-4 shadow-lg z-50 w-72">
                        ...
                        <div className="mb-4">
                            <SelectOptions value={brand} options={[{label: "all", value: ""}, ...brandOptions.map(b => ({label: b, value: b.toLowerCase()}))]} onChange={setBrand} options_name={t("adminpanel.cars.filter.car_filter")} multiple={true}/>
                        </div>

                        <div className="mb-4">
                            <SelectOptions value={segment} options={[{label: "all", value: ""}, ...segmentsOptions.map(s => ({label: t(s.translation_key.key), value: t(s.translation_key.key).toLowerCase()}))]} onChange={setSegment} options_name={t("adminpanel.cars.filter.segment")} multiple={true}
                            />
                        </div>
                        <div className="mb-4">
                            <SelectOptions value={bodyType} options={[{label: "all", value: ""}, ...bodyTypeOptions.map(b => ({label: t(b.translation_key.key), value: t(b.translation_key.key).toLowerCase()}))]} onChange={setBodyType} options_name={t("adminpanel.cars.filter.body_type")} multiple={true}/>
                        </div>
                        <div className="mb-4">
                            <SelectOptions value={transmission} options={[{label: "all", value: ""}, ...transmissionOptions.map(a => ({label: t(a.translation_key.key), value: t(a.translation_key.key).toLowerCase()}))]} onChange={setTransmission} options_name={t("adminpanel.cars.filter.transmission_type")} multiple={true}/>
                        </div>
                        <div className="mb-4">
                            <SelectOptions value={fuelType} options={[{label: "all", value: ""}, ...fuelOptions.map(f => ({label: t(f.translation_key.key), value: t(f.translation_key.key).toLowerCase()}))]} onChange={setFuelType} options_name={t("adminpanel.cars.filter.fuel_type")} multiple={true}/>
                        </div>
                        <div className="mb-4">
