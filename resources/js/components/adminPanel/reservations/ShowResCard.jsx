...
const DetailItem = ({ label, value, className = "" }) => (
    <div className={`flex flex-col py-3 border-b border-gray-100 last:border-0 ${className}`}>
        <dt className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-1.5">{label}</dt>
        <dd className="text-sm text-gray-900 font-medium wrap-break-words">{value || "—"}</dd>
    </div>
);
...
export default function ShowResCard({ res, updateData, closeModal, curr, past }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 transition-all">
            <div className="bg-white w-full max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl flex flex-col relative border border-gray-200">

                <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200 sticky top-0 bg-white/98 backdrop-blur-sm z-10">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">
                            {t("adminpanel.reservation.reservation_modal.reservation_detail")}
                        </h2>
                        <p className="text-sm text-gray-500 mt-1">ID: #{res.reference_code}</p>
                    </div>
                    <button
                        onClick={() => closeModal()}
                        className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-gray-700 transition-colors cursor-pointer hover:shadow-sm"
                    >
                ...
                <div className="p-6 bg-white grow">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
                        ...
                        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
                            ...
                            {res.carGroup?.photos?.length > 0 && (
                                <div className="mb-4 rounded-lg overflow-hidden h-40 w-full bg-gray-200 border border-gray-300 shadow-sm">
                                    <img
                        ...
                        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200">
                            <div className="flex justify-center items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                        ...
                        <div className="bg-gradient-to-br from-gray-50 to-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col hover:shadow-md transition-shadow duration-200">
                            <div className="flex justify-center items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                            ...
                            <dl className="flex flex-col gap-1 grow">
                                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                                    <dt className="text-xs font-semibold text-gray-600 uppercase">{t("adminpanel.reservation.reservation_modal.payment_status")}</dt>
                                    <dd className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusStyle(res.payment_status)}`}>
                                        {res.payment_status}
                                    </dd>
                                </div>
                                ...
                                {res?.online_discount_amount > 0 && <DetailItem label={t("adminpanel.reservation.reservation_modal.online_discount_amount", "Online Discount Amount")} value={`% ${(res.online_discount_amount * 100).toFixed(2)}`} />}
