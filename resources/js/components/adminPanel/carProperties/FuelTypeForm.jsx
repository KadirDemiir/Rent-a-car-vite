...
export default function FuelTypeForm({mode="create", lngs, fuel=null}){
    ...
    const [formData, setFormData] = useState(() => {
        const nameObj = lngs.reduce((acc, lang) => {
            acc[lang.code] = { value: fuel?.translation_key_id ? t(`fuel.${fuel.id}`) :  "", error: "" };
            return acc;
        }, {});
        return { name: nameObj };
    });
    ...
    const handleSubmit = () => {
        ...
        const url = mode === 'create' ? '/adminpanel/fuels/add' : `/adminpanel/fuels/${fuel?.id}`;
        const request =
            mode === "create"
                ...
                if (response.data.success) {
                    if(response.data.translations){
                        Object.entries(response.data.translations).forEach(([langId, tVal]) => {
                            ...
                            if(code) i18next.addResource(code, 'translation', `fuel.${response.data.fuel_id}`, tVal.value);
                        });
                    }
