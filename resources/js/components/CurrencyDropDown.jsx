...
export default function CurrencyDropDown() {
    ...
    return (
        <div ref={dropdownRef} className="w-32 relative flex flex-col items-center">
            <button type={`button`} onClick={() => setOpen(!open)} className="w-full px-2 py-1.5 border border-blue-400/60 bg-blue-500 text-white rounded-xl flex items-center justify-between">
            ...
            {open && (
                <ul className="absolute top-full mt-2 bg-white border border-blue-200 rounded-lg shadow-lg w-full z-40 text-center max-h-60 overflow-auto">
                    {currencies.map(curr => (
                        <li
                            key={curr.id} className={`flex items-center justify-center gap-2 px-3 py-2 text-sm cursor-pointer ${current.code === curr.code ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-blue-50'}`} onClick={() => { changeCurrency(curr); setOpen(false); }}>
