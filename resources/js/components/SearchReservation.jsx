...
export default function SearchReservation() {
    ...
    const locations = ["Ankara", "İstanbul", "Antalya", "İzmir", "Nevşehir", "Kayseri"];
            ...
            <form onSubmit={submitHandler} action="" className="flex items-center justify-center  gap-4">
                ...
                <div className="relative w-80">
                    <div className="w-full">
                        ...
                        <button type="button" onClick={() => setIsPUOpen(!isPUOpen)} className="border w-full h-12 text-xl font-semibold flex items-center justify-start pl-4 rounded-md">
                            {selectedPULocation}
                        </button>
                        {isPUOpen && (
                            <div className="absolute z-10 mt-2 w-full bg-white shadow-lg rounded-md p-4"  onBlur={() => setIsPUOpen(false)} tabIndex={-1}>
                                ...
                                {locations.map((location) => (
                                    <div key={location} className="h-10 mt-2">
                                        <button type="button" className="h-full cursor-pointer w-full" 
                                         onClick={() => {
                                            setSelectedPULocation(location);  
                                            setIsPUOpen(false);              
                                          }}
                                        >
                                            {location}
                ...
                <div className="relative w-80">
                    <div className="w-full">
                        ...
                        <button type="button" onClick={() => setIsROpen(!isROpen)} className="border w-full h-12 text-xl font-semibold flex items-center justify-start pl-4 rounded-md">
                            {selectedRLocation}
                        </button>
                        {isROpen && (
                            <div className="absolute z-10 mt-2 w-full bg-white shadow-lg rounded-md p-4"  onBlur={() => setIsROpen(false)} tabIndex={-1}>
                                <input type="text" className="w-full p-2 border rounded-md outline-none" placeholder="Lokasyon ara..." />
                                {locations.map((location) => (
                                    <div key={location} className="h-10 mt-2">
                                        <button type="button" className="h-full cursor-pointer w-full" 
                                         onClick={() => {
                                            setSelectedRLocation(location);  
                                            setIsROpen(false);              
                                          }}
                                        >
                                            {location}
                ...
                <div className="flex gap-4">
                    <div>
                        ...
                        <div className="h-12 w-72 border rounded-md flex">
                            ...
                            <div className="flex item-center">
                                <select className="outline-none" value={startClock} onChange={(e) => setStartClock(e.target.value)}>
                                    {clocks.map((clock) => 
                    ...
                    <div>
                        ...
                        <div className="h-12 w-72 border rounded-md flex">
                            <DatePicker
                            minDate={new Date(finishDate.setDate(finishDate.getDate() + 1))}
                            selected={finishDate}
                            ...
                            <div className="flex item-center">
                                <select className="outline-none" value={finishClock} onChange={(e) => setFinishClock(e.target.value)}>
                                    {clocks.map((clock) => 
                                    <option key={clock} value={clock} >{clock}</option>
                                    )}
                                </select>
