...
// 1. DEĞİŞİKLİK: Dilleri bileşen render olmadan önce, en tepede kaydediyoruz.
registerLocale('tr', tr);
registerLocale('en', enUS);

// Desteklenen dilleri burada tanımlıyoruz (Fallback mantığı için)
const supportedLocales = ['tr', 'en'];

const CustomInput = forwardRef(({ value, onClick }, ref) => (
  <input className="w-full h-12 px-4 rounded-xl outline-none cursor-pointer transition-colors text-gray-800 font-medium" onClick={onClick} value={value} ref={ref} readOnly/>
));
...
export default function SearchReservation({locations, defPickupLocation, defReturnLocation, defReturnDate, defReturnClock, defPickupClock, defPickupDate, home=true }) {
  ...
  // 2. DEĞİŞİKLİK: registerLocale useEffect'i kaldırıldı.
  ...
  // 3. DEĞİŞİKLİK: Geçerli dil kontrolü
  // Eğer i18n.language desteklenenler listesinde varsa onu kullan, yoksa 'en' yap.
