    ...
    return(
        <div className={`w-full rounded-lg shadow-lg bg-white flex flex-col gap-4`}>
            ...
            <div className={`flex items-center justify-between px-4`}>
                <span>Car Price: </span>
                <span>{`${calculateTotal(total_days)} x ${calculateTotal(daily_price)} ${current?.symbol}`}</span>
            </div>
            <div className={`flex items-center justify-between px-4`}>
                <span>Extra Prices: </span>
                <span>{`${calculateTotal(extra_price)} ${current?.symbol}`}</span>
            </div>
            <div className={`flex items-center justify-between px-4 pb-2`}>
                <span className={`font-bold`}>Total: </span>
                <span>{`${calculateTotal(total_days*daily_price + extra_price)} ${current?.symbol}`}</span>
            </div>
        </div>
    );
}
