        ...
        <div ref={wrapperRef} className="flex flex-col w-full relative">
          ...
          {open && (
            <ul className="absolute top-full left-0 w-full bg-white border border-gray-300 rounded-md shadow-md z-10 max-h-80 overflow-auto">
              {options.map((opt) => (
                <li key={opt.value} className={`px-4 py-2 hover:bg-gray-100 cursor-pointer flex justify-between items-center ${isSelected(opt.value) ? "bg-gray-100 font-semibold" : ""}`} onClick={() => toggleSelection(opt.value)}>
                    {typeof opt.label === "string" ? (opt.label) : (React.isValidElement(opt.label) ? opt.label : opt.label)}
